...
const employers = ['Alex', '', 'ludmila', 'Viktor', '', 'oleg',
	'iNna', 'Ivan', 'Alex', 'Olga', ' Ann'];
const employersNames = [];

employers.forEach((name) => {
	if (name.length > 0 && name.length != "") {
		name = name.toLowerCase().trim();//Нижний регистр и убраны пробельные символы
		employersNames.push(name);
	}
});
